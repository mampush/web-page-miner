...
def get_records(domain):
    results = {}
    record_types = ['NONE', 'A', 'NS', 'MD', 'MF', 'CNAME', 'SOA', 'MB', 'MG', 'MR', 'NULL', 'WKS', 'PTR', 'HINFO',
                    'MINFO', 'MX', 'TXT', 'RP', 'AFSDB', 'X25', 'ISDN', 'RT', 'NSAP', 'NSAP-PTR', 'SIG', 'KEY', 'PX',
                    'GPOS', 'AAAA', 'LOC', 'NXT', 'SRV', 'NAPTR', 'KX', 'CERT', 'A6',
                    'DNAME', 'OPT', 'APL', 'DS', 'SSHFP', 'IPSECKEY', 'RRSIG', 'NSEC', 'DNSKEY', 'DHCID', 'NSEC3',
                    'NSEC3PARAM', 'TLSA', 'HIP', 'CDS', 'CDNSKEY', 'CSYNC', 'SPF', 'UNSPEC', 'EUI48', 'EUI64', 'TKEY',
...
def get_internal_external_links(url, timeout=5):
    try:
        ...
        return {'error': f'Request error during request to {e}',
                'message': 'Some sites might prohibit automated requests.'}
...
def get_phone_numbers(url, timeout=5):
    try:
        ...
        phone_numbers = re.findall(
            r'\(\+\d{1,3}\)\s?\d{3}\s?\d{6}|\d{3}\s?\d{1}\s?\d{3}\s?\d{3}', text_content)
